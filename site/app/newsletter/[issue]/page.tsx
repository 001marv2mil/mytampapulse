...
export default async function NewsletterIssuePage({ params }: PageProps) {
  ...
  // All issues are gated — subscribers only
  {
    return (
      ...
      <article className="max-w-2xl mx-auto px-6">
                  ...
                  {newsletter.weatherTable.map((row, i) => (
                    <tr key={i} className="border-t border-gray-100">
                      ...
                      <td className="py-2.5 px-3">
                        <span className={`weather-emoji ${getWeatherEmojiClass(row.emoji)}`}>
                          {row.emoji}
                        </span>
                      </td>
        ...
        {newsletter.images[0] && (
          <div className="mb-12 rounded-2xl overflow-hidden shadow-sm">
            <img src={newsletter.images[0].src} alt={newsletter.images[0].alt}
              className="w-full h-56 sm:h-72 object-cover" />
            {newsletter.images[0].alt && (
              <p className="text-gray-400 text-xs mt-2 italic px-1">
        ...
        {newsletter.images[1] && (
          <div className="mb-12 rounded-2xl overflow-hidden shadow-sm">
            <img src={newsletter.images[1].src} alt={newsletter.images[1].alt}
              className="w-full h-56 sm:h-72 object-cover" />
              ...
              {newsletter.communityPickBody.map((para, i) => {
                ...
                return paragraphs.map((p, j) => {
                  ...
                  if (trimmed.startsWith("*") && trimmed.endsWith("*") && !trimmed.startsWith("**")) {
                    return (
                      <p key={`${i}-${j}`} className="text-gray-400 text-sm leading-relaxed italic">
                        {stripEmDashes(trimmed.replace(/^\*|\*$/g, ""))}
                      </p>
                    );
                  }
                  return (
                    <p key={`${i}-${j}`} className="text-gray-600 text-sm leading-relaxed">
        ...
        {newsletter.images[2] && (
          <div className="mb-12 rounded-2xl overflow-hidden shadow-sm">
            <img src={newsletter.images[2].src} alt={newsletter.images[2].alt}
              className="w-full h-56 sm:h-72 object-cover" />
          ...
          <section className="mb-12 text-center bg-orange-50 rounded-2xl p-8">
            {newsletter.signoff.split("\n").filter((l) => l.trim()).map((line, i) => {
              ...
              if (clean.toLowerCase().includes("marv")) {
                return <p key={i} className="font-heading font-bold text-gray-900 text-lg">{clean}</p>;
              }
              if (clean.toLowerCase().includes("see you") || clean.toLowerCase().includes("next thursday") || clean.toLowerCase().includes("next week")) {
                return <p key={i} className="text-gray-500 text-sm mb-2">{clean}</p>;
              }
        ...
        <div className="flex items-center justify-between mb-12">
          {prevIssue ? (
            <Link href={`/newsletter/${prevIssue}`}
              className="text-gray-400 hover:text-pulse-orange transition-colors text-sm">
          ...
          {nextIssue ? (
            <Link href={`/newsletter/${nextIssue}`}
              className="text-gray-400 hover:text-pulse-orange transition-colors text-sm">

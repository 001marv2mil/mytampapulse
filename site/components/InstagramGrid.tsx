        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {posts.map((post, i) => (
            <motion.a key={post.id} href="https://instagram.com/mytampapulse" target="_blank" rel="noopener noreferrer"
              ...
              className="group relative aspect-square rounded-2xl overflow-hidden bg-orange-50 border border-orange-100 hover:border-pulse-orange/40 transition-colors duration-300">
              <Image src={post.img} alt={post.caption} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
              <div className="absolute inset-0 bg-gradient-to-t from-orange-950/60 via-pulse-orange/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                <span className="text-orange-200 text-[10px] font-bold uppercase tracking-wider block mb-1">{post.tag}</span>
                <p className="text-white text-xs font-medium leading-tight line-clamp-2">{post.caption}</p>
                <p className="text-orange-200/70 text-[10px] mt-1">❤️ {post.likes}</p>
              </div>
            </motion.a>
          ))}
        </div>
